...
export default function ChatbotWidget() {
    ...
    const handleSend = async (e) => {
        ...
        try {
            const res = await sendChatMessage({ message: userMsg, language });
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 font-sans">
            {isOpen ? (
                <div className="glass-card neon-border w-80 sm:w-96 h-[30rem] flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 zoom-in-95 duration-200" style={{ backgroundColor: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(10px)', borderRadius: '1rem', border: '1px solid rgba(139, 92, 246, 0.3)' }}>
                    {/* Header */}
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        className="bg-slate-800 text-white text-[10px] rounded px-1.5 py-0.5 outline-none border border-slate-600 cursor-pointer"
                                    >
                    ...
                    <form onSubmit={handleSend} className="p-3 flex gap-2 border-t border-slate-700/50" style={{ backgroundColor: '#0f172a' }}>
                        ...
                        <button
                            ...
                            className="bg-primary-600 text-white w-10 h-10 rounded-xl flex items-center justify-center hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:bg-slate-700"
                        >
