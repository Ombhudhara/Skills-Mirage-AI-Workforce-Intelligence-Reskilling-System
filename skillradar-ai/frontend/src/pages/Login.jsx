...
export default function Login() {
    ...
    const handleLogin = async (e) => {
        ...
        // Rapid Bypass: Direct entry to dashboard as requested
        setTimeout(() => {
            localStorage.setItem('isAuthenticated', 'true');
            localStorage.setItem('user', JSON.stringify({ email: email || 'user@mirage.ai', name: 'Neural Operator' }));
            navigate('/dashboard');
            setIsLoading(false);
        }, 800);
    };
            ...
            <div className="relative z-10 w-full max-w-md">
                ...
                <div className="glass-card p-8 neon-border">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                        <button onClick={() => window.location.href = 'http://localhost:8001/api/auth/google'}
                            className="py-3 px-4 bg-slate-900/5 hover:bg-slate-900/10 border border-white/10 rounded-xl text-white text-[11px] font-bold tracking-wider flex items-center justify-center gap-2 transition-all hover:border-white/20">
                        ...
                        <button onClick={() => window.location.href = 'http://localhost:8001/api/auth/github'}
                            className="py-3 px-4 bg-slate-900/5 hover:bg-slate-900/10 border border-white/10 rounded-xl text-white text-[11px] font-bold tracking-wider flex items-center justify-center gap-2 transition-all hover:border-white/20">
                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
